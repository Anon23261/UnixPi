...
class SystemMonitor:
    ...
    async def _check_anomalies(self, sample: Dict, results: Dict):
        ...
        # Network connections
        if sample['network']['connections'] > self.baseline['network']['connections'] * 2:
            results['anomalies'].append({
                'timestamp': sample['timestamp'],
    ...
    async def _security_assessment(self, results: Dict):
        ...
        for proc in psutil.process_iter(['name', 'username', 'cmdline']):
            try:
                ...
                if any(mal in proc.info['name'].lower() 
    ...
    def generate_report(self, results: Dict, filename: str = "system_monitor_report.json"):
    ...
    def _calculate_risk_level(self, anomalies: List[Dict], 
                            security_issues: List[Dict]) -> str:
